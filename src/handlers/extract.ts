...
import { restricted, getClient } from "../utils/helpers";
import { TelegraphClient } from "../telegraphClient";
import type { TransfersDict, Transfer } from "../types";
...
const CANCEL_FLAGS: Record<number, boolean> = {};

export const cancelCommand = async (ctx: Context): Promise<void> => {
  const userId = ctx.from!.id;
  CANCEL_FLAGS[userId] = true;
  await ctx.reply("🛑 Cancellation request sent.");
};

export const extract = restricted(async (ctx: Context) => {
  const userId = ctx.from!.id;
  ...
  if (transfers?.error) {
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, `Error: ${transfers.error}`);
    return;
  }

  let allFinished: Transfer[] = [];
  if (Array.isArray(transfers)) {
    allFinished = transfers;
  } else if (transfers && typeof transfers === "object") {
    allFinished = (transfers as TransfersDict).finished ?? [];
  }

  if (!allFinished.length) {
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, "No transfers found.");
    return;
  }

  const text = ctx.message?.text ?? "";
  const argsStr = text.split(" ").slice(1).join("").replace(/\s/g, "");
  let selectedTransfers: Transfer[] = [];
  ...
  if (argsStr) {
    ...
    for (const idx of [...indices].sort((a, b) => a - b)) {
      if (idx >= 1 && idx <= allFinished.length) selectedTransfers.push(allFinished[idx - 1]);
    }
    if (!selectedTransfers.length) {
      await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, "No valid transfers selected from range.");
      return;
    }
  } else {
  ...
  const extractedItems: [string, string][] = [];
  let lastUpdateTime = Date.now();

  for (let i = 0; i < selectedTransfers.length; i++) {
    if (CANCEL_FLAGS[userId]) {
      await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, "❌ Extraction Cancelled by User.");
      return;
    }
    ...
    if ((data as any)?.status === "success" && (data as any).files) {
      const files = (data as any).files.content ?? [];
    ...
    if (now - lastUpdateTime > 2000 || i === total - 1) {
      ...
      try {
        await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, `Processing...\n${i + 1}/${total} [${bar}]`);
        lastUpdateTime = now;
      } catch (_) {}
    }
  }
  ...
  if (!extractedItems.length) {
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, "No suitable video files found.");
    return;
  }

  let pageUrl: string | null = null;
  if (extractedItems.length >= 5) {
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, "Creating Telegraph page...");
    ...
    try { pageUrl = await telegraph.createPage(title, nodes); } catch (e: any) { console.error("Telegraph creation failed:", e.message); }
  }

  if (pageUrl) {
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, `✅ Extraction Complete!\n\nView Links: ${pageUrl}`);
  } else {
    const prefix = extractedItems.length >= 5 ? "⚠️ Telegraph connection failed. Sending links directly..." : "✅ Extraction Complete! Sending links directly...";
    await ctx.api.editMessageText(ctx.chat!.id, statusMsg.message_id, prefix);
    let chunk = "";
    for (const [name, link] of extractedItems) {
      const line = `${name}\n${link}\n\n`;
      if (chunk.length + line.length > 4000) { await ctx.reply(chunk); chunk = ""; }
      chunk += line;
    }
