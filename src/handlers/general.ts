...
import {
  ...
  getActiveAccountExpiry,
  getActiveAccountTraffic,
} from "../utils/helpers";
import { TelegraphClient } from "../telegraphClient";
import { runBackup, restoreFromDropbox, getBackupInfo, isConfigured as isDropboxConfigured } from "../nzb/backup";
...
export const start = restricted(async (ctx: Ctx) => {
  await ctx.reply(
...
export const login = restricted(async (ctx: Ctx) => {
  const userId = ctx.from!.id;
  const args = (ctx as any).match
    ? String((ctx as any).match).trim()
...
export const versionCommand = async (ctx: Context) => {
  ...
  if (imgUrl) {
    try {
      ...
      await ctx.reply(message, { parse_mode: "HTML", disable_web_page_preview: false } as any);
      return;
    } catch (_) {}
  }
...
export const setCommand = restricted(async (ctx: Ctx) => {
...
export const cookiesCommand = restricted(async (ctx: Ctx) => {
  const userId = ctx.from!.id;
  ...
  const msg = await ctx.reply(`Fetching cookies for profile: \`${profileName}\`...`, { parse_mode: "Markdown" });

  const success = await fetchMagicCookies(profileName);
  if (success) {
    const client = getClient(userId);
    client.updateCookies(getMagicCookies());
    const renewRes = await client.renewFreeTrial();
    const renewed = renewRes?.success || renewRes?.message === "Trial activated";
    const email = getActiveAccountEmail();
    const expiry = getActiveAccountExpiry() || "Unknown";
    const traffic = getActiveAccountTraffic() || "unlimited";
    const infoStr = email ? `\nUser: **${email}**\nExpiry: **${expiry}**\nTraffic: **${traffic}**` : "";
    await ctx.api.editMessageText(
      ctx.chat!.id,
      msg.message_id,
    ...
    await ctx.api.editMessageText(ctx.chat!.id, msg.message_id, "❌ Failed to fetch cookies. Check logs.");
  }
});

export const setReplyHandler = restricted(async (ctx: Ctx) => {
...
export const approveCommand = async (ctx: Context) => {
  const userId = ctx.from!.id;
  ...
  if (!args.length) { await ctx.reply("Usage: /approve <user_id>"); return; }
  const targetId = parseInt(args[0], 10);
  if (isNaN(targetId)) { await ctx.reply("❌ Invalid user ID."); return; }
  if (approveUser(targetId)) {
    await ctx.reply(`✅ User <code>${targetId}</code> has been approved.`, { parse_mode: "HTML" });
  } else {
    await ctx.reply(`ℹ️ User <code>${targetId}</code> is already approved.`, { parse_mode: "HTML" });
  }
};

export const disapproveCommand = async (ctx: Context) => {
  const userId = ctx.from!.id;
  if (AUTHORIZED_USERS.length && !AUTHORIZED_USERS.includes(userId)) {
    await ctx.reply("⛔ Only bot owners can use this command.");
    return;
  ...
  const text = ctx.message?.text ?? "";
  const args = text.split(" ").slice(1).filter(Boolean);
  if (!args.length) { await ctx.reply("Usage: /disapprove <user_id>"); return; }
  const targetId = parseInt(args[0], 10);
  if (isNaN(targetId)) { await ctx.reply("❌ Invalid user ID."); return; }
  if (disapproveUser(targetId)) {
    await ctx.reply(`🚫 User <code>${targetId}</code> has been removed.`, { parse_mode: "HTML" });
  } else {
    await ctx.reply(`ℹ️ User <code>${targetId}</code> was not in the approved list.`, { parse_mode: "HTML" });
  }
};

export const approvedCommand = async (ctx: Context) => {
  const userId = ctx.from!.id;
  if (AUTHORIZED_USERS.length && !AUTHORIZED_USERS.includes(userId)) {
    await ctx.reply("⛔ Only bot owners can use this command.");
    return;
...
export const tdelCommand = restricted(async (ctx: Ctx) => {
  const reply = ctx.message?.reply_to_message;
  if (!reply) { await ctx.reply("REPLY to a message containing the Telegraph link."); return; }

  const text = (reply as any).text ?? (reply as any).caption ?? "";
  let match = text.match(/telegra\.ph\/([a-zA-Z0-9-]+)/);

  if (!match) {
    const entities = [...((reply as any).entities ?? []), ...((reply as any).caption_entities ?? [])];
    for (const ent of entities) {
      const url = (ent as any).url ?? "";
      const m = url.match(/telegra\.ph\/([a-zA-Z0-9-]+)/);
      if (m) { match = m; break; }
    }
  }

  if (!match) { await ctx.reply("No Telegraph link found in replied message."); return; }

  const path = match[1];
  const token = process.env.TELEGRAPH_TOKEN;
  if (!token) { await ctx.reply("❌ Telegraph Token not found on server."); return; }
...
export const renewCommand = restricted(async (ctx: Ctx) => {
  const msg = await ctx.reply("🔍 checking accounts...");
  const profiles = await getAvailableCookieProfiles();
  if (!profiles.length) {
    await ctx.api.editMessageText(ctx.chat!.id, msg.message_id, "❌ No profiles found.");
    return;
  }
  ...
  for (const profile of profiles) {
    ...
    if (!cookieStr) { results.push(`❌ \`${profile}\`: Failed to fetch cookies`); continue; }

    const tempClient = new MagicClient(cookieStr);
    const info = await tempClient.getAccountInfo();
    if (!info) { results.push(`⚠️ \`${profile}\`: Failed to get info`); continue; }
      ...
      try {
        ...
        if (!isNaN(expiry.getTime())) {
          if (expiry < new Date()) {
            ...
            const diff = Math.floor((expiry.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
            notes = `(${diff} days left)`;
          }
        } else {
          notes = `(Date: ${daysStr})`;
        }
      } catch (_) { notes = `(Date: ${daysStr})`; }
    }
...
export const backupCommand = restricted(async (ctx: Ctx) => {
...
export const restoreCommand = restricted(async (ctx: Ctx) => {
  if (!isDropboxConfigured()) { await ctx.reply("❌ Dropbox backup not configured."); return; }
